        ...
        for tr in all_tr_tags_list:
            ...
            for feilde in feildes:
                if feilde == 'origin_time':
                    ...
                    without_middel_space = re.sub(r',\s{1,2}', ',', without_ender_space)
                    earth_quakes[earth_quake]['region']=without_middel_space.split(',')
    ...
    for earth_quake in list(recent_earth_quake.items())[::-1]:
        if earth_quake[1]['origin_time'] > last_data_date:
            EarthQuake(origin_time=earth_quake[1]['origin_time'],
                       ...
                       region_city=earth_quake[1]['region'][0],
                       region_state=earth_quake[1]['region'][1]).save()

# use when db table is empty
def init_data():
    recent_earth_quake = scraper()
    for earth_quake in list(recent_earth_quake.items())[::-1]:
        EarthQuake(origin_time=recent_earth_quake[earth_quake[0]]['origin_time'],
                   ...
                   region_city=recent_earth_quake[earth_quake[0]]['region'][0],
                   region_state=recent_earth_quake[earth_quake[0]]['region'][1]).save()
